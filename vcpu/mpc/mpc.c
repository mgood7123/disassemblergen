...
mpc_parser_t *mpc_re_mode(const char *re, int mode) {
  ...
  mpc_define(Base, mpc_or(4,
	/*
	parens accept Regex as rules to parse
	
	to accept the regex of lookarounds we need to first check for "?<mode>" and modify the parser accordingly
	
	?! negates expects, 	eg [^a]
	?: expects, 			eg [a]
	if none is specified we assume standard Regex rules
	
	note:
	in order to clone an input we do:
